...
exports.lambdaHandler = async (event) => {
  ...
  try {
    ...
    switch (intentName) {
      case "ViewBookingInformation":
        return await handleViewBookingInformation(event);
...
async function handleCreateReservation(event) {
  ...
  const reservationDate =
    event.sessionState.intent.slots.reservation_date.value.interpretedValue;
  const reservationTime =
    event.sessionState.intent.slots.reservation_time.value.interpretedValue;
  const customerId =
  ...
  if (response.success) {
    return buildResponse(
      event,
      `Your reservation at ${restaurantName} has been created.`
    );
  } else {
    return buildResponse(
      event,
      "Sorry, there was an error creating your reservation."
    );
  }
}

async function createReservation(restaurantId, unixTimestamp, customerId) {
  const data = {
    restaurant_id: restaurantId,
    reservation_time: unixTimestamp,
