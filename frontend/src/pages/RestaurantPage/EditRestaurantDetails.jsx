...
function restaurant() {
  ...
  async function saveEditChanges() {
    ...
    if (restaurant.discount_percentage !== 0) {
      ...
      const offerDescription = "The restaurant \"" + restaurant.name + "\" has the discount of " + restaurant.discount_percentage + " on all the restaurant items";
      const offer = {
        offer_id: uuidValue,
        is_new: true,
        offer_on: restaurant.offer_on,
      ...
      await fetch(`https://e4x258613e.execute-api.us-east-1.amazonaws.com/offers`, putOptions);
    }
  ...
  async function saveMenuEditChanges() {
    ...
    if (restaurant.offer_on === "menu_item") {
      for (const item of restaurant.menu) {
        if (!item.item_size_price) {
          continue;
        }
        for (const temp of item.item_size_price) {
          if (parseInt(maxDiscountVal) < parseInt(temp.discount_percentage)) {
            maxDiscountVal = temp.discount_percentage;
          }
        }
      }
      if (maxDiscountVal !== '0') {
        const uuidValue = uuidv4();
        const offerDescription = "The restaurant \"" + restaurant.name + "\" has the maximum discount of " + maxDiscountVal + " percentage on selected menu items";
        const offer = {
          offer_id: uuidValue,
          is_new: true,
          offer_on: restaurant.offer_on,
          ...
          <Box bg="white" p="20px" rounded="md">
            ...
            {inEditingMode ? (
              ...
              <Flex justifyContent="space-between" alignItems="center">
                ...
                <NavLink to = {`/partner/reservations/restaurants/${restaurant_id}`} >
                  <Button ml="40px" mt="15px" colorScheme='yellow'> View Reservations </Button>
                </NavLink>
              </Flex>
            )}
              ...
              <Text p="5px" fontSize="lg">
                ...
                {inEditingMode ? (
                  <input type="checkbox" name="is_offer" checked={restaurant.is_offer}
                    onChange={(event) =>
                      setRestaurant({
                        ...restaurant, is_offer: event.target.checked, // Set the boolean is_offer flag
